...
carapace.debug = false;

carapace.log = function (data) {
  ...
  if (carapace.debug) {
    console.log('info: ' + ( data.carapace.pid || null ) + ' ' + data.event + '    ' + JSON.stringify(data.data));
  }

};

carapace.onAny(function (data) {
  carapace.log({ event: this.event, data: data, carapace: { pid: carapace.pid }});
});
...
//
// Set the carapace id to something unique, like it's process id
//
carapace.id = process.pid;
//
...
carapace.start = function (callback) {
  carapace.on('drone::ready', callback);
  carapace.run();
};
...
carapace.run = function (override, callback) {
  ...
  else if (!path.existsSync(carapace.script)) {
    error = new Error('Cannot find script ' + carapace.script);
  }
  ...
  process.nextTick(function () {
    ...
    carapace.on('port::bound', function(info) {
      carapace.port = info.port;
      carapace.emit('drone::listening', info.port);
      carapace.emit('drone::ready');
    });

    carapace.running = true;
    carapace.emit('drone::running', {
      id: carapace.id,
      script: carapace.script,
      argv: carapace.argv 
    });
