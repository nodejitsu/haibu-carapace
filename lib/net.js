...
function nextPort(port) {
  ...
  //
  var unavailable = drone.ports.ignore.concat(drone.ports.throw);
  do {
    port = port + 1;
...
// server (the `this` argument) is in `drone.ports.throw`
//
function reservedPort(desired) {
  if (drone.ports.throw.indexOf(desired) !== -1) {
    this.close();
    //
    // Build fake error
    //
  ...
  net.Server.prototype._doListen = function ourListen() {
    ...
    for (;;) {
      try {
        ...
        // `drone.ports.throw` which should always throw an error,
    ...
    process.nextTick(function () {
      ...
      try {
        ...
        // on the drone itself, indexed by port.
        //
        drone.servers[desired] = self;

        drone.emit('port::requested', {
          id: drone.id,
          addr: addr,
          desired: desired,
          port: actual
...
exports._listen2 = function overrideNet() {
  ...
  net.Server.prototype._listen2 = function ourListen(address, port, addressType, desired) {
    ...
    // that we first emit `drone::port` and *then* the proper `listening`
